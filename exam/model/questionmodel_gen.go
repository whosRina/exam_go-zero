...
type (
	questionModel interface {
		...
		GetQuestionsByType(ctx context.Context, bankId int64, qType int) ([]*Question, error)
	}
...
func (m *defaultQuestionModel) GetQuestionsByType(ctx context.Context, bankId int64, qType int) ([]*Question, error) {
	query := fmt.Sprintf("select %s from %s where bank_id = ? and type = ?", questionRows, m.table)
	var questions []*Question
	err := m.conn.QueryRowsCtx(ctx, &questions, query, bankId, qType)
	switch err {
	case nil:
