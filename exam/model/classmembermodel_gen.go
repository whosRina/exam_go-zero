...
type (
	classMemberModel interface {
		...
		FindByUser(ctx context.Context, userId int64) ([]int64, error)
		IsMemberInTeacherClass(ctx context.Context, teacherId, memberId int) (bool, error)
		FindByClassId(ctx context.Context, classId int64) ([]*ClassMember, error)
	}
...
func (m *defaultClassMemberModel) FindByUser(ctx context.Context, userId int64) ([]int64, error) {
	query := "SELECT `class_id` FROM class_member WHERE `user_id` = ?"
	var classIds []int64
	err := m.conn.QueryRowsCtx(ctx, &classIds, query, userId)
	if err != nil {
...
// FindByClassId 查询某个班级的所有成员
func (m *defaultClassMemberModel) FindByClassId(ctx context.Context, classId int64) ([]*ClassMember, error) {
	query := fmt.Sprintf("SELECT id, class_id, user_id FROM %s WHERE class_id = ?", m.table)

	var members []*ClassMember
	err := m.conn.QueryRowsCtx(ctx, &members, query, classId)
	if err != nil {
		return nil, err
	}
